...
input.isGesture(Gesture.LogoUp)
...
radio.onReceivedValue(function(n,v){})
...
pins.i2cReadNumber(0x10, NumberFormat.UInt8LE, false)
pins.i2cWriteNumber(0x10, 5, NumberFormat.UInt8LE, false)

// SERIAL
serial.writeLine("test")
serial.writeNumber(123)
serial.writeValue("t", 3)
serial.readString()
...
let s = sprite.get(LedSpriteProperty.Brightness)
game.addScore(1)
game.score()
game.over()
```